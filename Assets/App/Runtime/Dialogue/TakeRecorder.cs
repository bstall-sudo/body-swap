...
namespace App.Runtime.Dialogue
{
    public class TakeRecorder
    {
        ...
        private readonly Transform _actorRoot;     // ActorA oder ActorB
        private readonly IInputProvider _input;    // XRInputProvider (Anchor schon gesetzt)
        ...
        private static string PickMicDevice()
        {
            ...
            // Bevorzugt Headset-Mics (Vive/HTC), sonst erstes Gerät
        ...
        private static int PickSupportedSampleRate(string device, int preferred = 48000)
        {
            ...
            // Manche Treiber geben 0/0 zurück -> dann nimm Unity OutputSampleRate
            if (minHz == 0 && maxHz == 0)
...

        {
            ...
            _actorRoot.localRotation = Quaternion.Euler(0f, yaw, 0f);

            // Head/Hands relativ zum ActorRoot speichern:
            // stage-local -> actor-local
            var actorRot = _actorRoot.localRotation;
        ...
        public (AudioClip clip, int startSample, int sampleCount, int channels, int sampleRate)? EndAndGetTrimInfo()
        {
            ...
            // Wichtig: hier KEIN GetData, nur Info zurückgeben
